...
fn main() -> Result<(), Box<dyn Error + 'static>> {
    ...
    for line in INPUT.lines() {
        let mass = line?.parse()?;
        total_fuel += fuel_from_mass_fixed_point(mass);
    }
    println!("{}", total_fuel);
    Ok(())
}

fn fuel_from_mass_fixed_point(mut mass: usize) -> usize {
    let mut total_fuel = 0;
    loop {
        let extra_fuel = fuel_from_mass(mass);
        if extra_fuel == 0 {
            break;
        }
...
mod tests {
    ...
    #[test]
    fn test_fuel_calc_fixed_point() {
        let mass_inputs = [14, 1969, 100756];
        let expected_outputs = [2, 966, 50346];

        for (&input, &output) in Iterator::zip(mass_inputs.iter(), expected_outputs.iter()) {
            assert_eq!(fuel_from_mass_fixed_point(input), output);
        }
    }
}
